...
return code
   ...
   // Reading the main ELF header.
	...
	printf("Not an ELF file. Magic: %x %x %x %x\n", magic[0], magic[1], magic[2], magic[3]);
	free_and_return(-1);
    }
   ...
   // Parsing sections to find relocation sections.
    for (i = 0; i < head.e_shnum; i++) {
	    ...
	    if (sec[i].sh_type != REL && sec[i].sh_type != 4/*RELA*/)
	        continue;
	    ...
	    if (sec[i].sh_entsize != sizeof(struct elf_reloc_t) && sec[i].sh_entsize != sizeof(elf_rela_t)) {
